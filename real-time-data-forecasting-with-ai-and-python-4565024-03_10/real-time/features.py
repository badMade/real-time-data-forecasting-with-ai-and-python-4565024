...
def calculate_lags(window):
    ...
    features = {
        ...
        "rolling_mean_7": (
            float(df["value"].rolling(window=7).mean().iloc[-1])
            if len(df) >= 7
            else None
        ),
        "rolling_std_7": (
            float(df["value"].rolling(window=7).std().iloc[-1])
            if len(df) >= 7
            else None
        ),
    }
...
def feature_pipeline_online(value, state, producer, feature_store_topic):
    ...
    if value["Type"] == "energy":
        ...
        message = {"id": feature_record_id, **features}

        producer.produce(
            topic=feature_store_topic.name,
            key=feature_record_id,
            value=json.dumps(message),
        )

    elif value["Type"] == "temperature":
        message = {"id": feature_record_id, "temperature_forecast": value["Value"]}
        producer.produce(
            topic=feature_store_topic.name,
            key=feature_record_id,
            value=json.dumps(message),
...
with app.get_producer() as producer:
    ...
    data_df = app.dataframe(topic=incoming_data_topic).update(
        lambda value, state: feature_pipeline_online(
            value, state, producer, feature_store_topic
        ),
        stateful=True,
    )
    app.run(data_df)
